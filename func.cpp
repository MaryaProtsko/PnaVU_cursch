    ...
    for (int i = 0; i < size; i++) {
        for (int j = i + 1; j < size; j++) {
            ...
            if (time1 > time2) {
                ...
                swapLinesInFile("C:/ToDo/ToDo/tag/" + date + ".txt", i, j);
                swapLinesInFile("C:/ToDo/ToDo/time/" + date + ".txt", i, j);
                QString way1 = QString::number(i);
                QString way2 = QString::number(j);
                swapFileContents("C:/ToDo/ToDo/note/" + date + "task" + way1 + ".txt",
                                 "C:/ToDo/ToDo/note/" + date + "task" + way2 + ".txt");
                swapItems(i, j, list);
            }
        }
    }
...
void deleteDateFile(int index, QString date, int count)
{
    if(count == index)
    {
        QString noteIndex = "C:/ToDo/ToDo/note/" + date + "task" + QString::number(index) + ".txt";
    ...
    {
        QString noteIndex = "C:/ToDo/ToDo/note/" + date + "task" + QString::number(i) + ".txt";
        QFile fileIndex(noteIndex);
        if (fileIndex.open(QIODevice::WriteOnly | QIODevice::Text)) {
            QString noteNextIndex = "C:/ToDo/ToDo/note/" + date + "task" + QString::number(i + 1) + ".txt";
