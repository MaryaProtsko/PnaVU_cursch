	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (unsigned int i = 0; i < 10; i++)
		{
			...
			if (shot) {
				...
				glm::vec3 rayOrigin = camera.Position; // Позиция камеры
				glm::vec3 rayDirection = calculateRayDirection(lastX, lastY, width, height, projection, view);

				// Параметры объекта
				glm::vec3 sphereCenter = armMob[i].getPosition(); // Центр объекта
				float sphereRadius = 1.5f; // Радиус объекта
...
glm::vec3 calculateRayDirection(float mouseX, float mouseY, int screenWidth, int screenHeight,
	const glm::mat4& projectionMatrix, const glm::mat4& viewMatrix) {
	// 1. Преобразуем координаты мыши в нормализованные координаты устройства (NDC)
	float x = (2.0f * mouseX) / screenWidth - 1.0f;
	float y = 1.0f - (2.0f * mouseY) / screenHeight;
	float z = 1.0f;  // Указываем направление вдоль оси Z
	glm::vec3 rayNDC(x, y, z);

	// 2. Преобразуем координаты в пространство клиппинга
	glm::vec4 rayClip(rayNDC.x, rayNDC.y, -1.0f, 1.0f);

	// 3. Преобразуем координаты в пространство камеры
	glm::vec4 rayEye = glm::inverse(projectionMatrix) * rayClip;
	rayEye = glm::vec4(rayEye.x, rayEye.y, -1.0f, 0.0f);

	// 4. Преобразуем координаты в мировое пространство
...
bool intersectRaySphere(const glm::vec3& rayOrigin, const glm::vec3& rayDirection,
	...
	return (discriminant > 0); // Если дискриминант положителен, то пересечение есть
}